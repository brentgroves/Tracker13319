...
async function InsToolLifeHistory(
  ...
  try {
    ...
    console.log(
      `In InsToolLifeHistory with params CNC_Approved_Workcenter_Key=${CNC_Approved_Workcenter_Key},Set_No=${Set_No},Block_No=${Block_No},Run_Quantity=${Run_Quantity},Run_Date=${Run_Date}`
    );
    ...
    const someRows = await conn.query(
      "call InsToolLifeHistory(?,?,?,?,?,@Tool_Life_Key,@Return_Value); select @Tool_Life_Key as pTool_Life_Key,@Return_Value as pReturn_Value",
      [CNC_Approved_Workcenter_Key, Set_No, Block_No, Run_Quantity, Run_Date]
    );
...
async function InsToolLifeHistoryV2(
  CNC_Approved_Workcenter_Key,
  Tool_Var,
  Run_Quantity,
  Run_Date
  ...
  try {
    conn = await pool.getConnection();
    console.log(
      `In InsToolLifeHistoryV2 with params CNC_Approved_Workcenter_Key=${CNC_Approved_Workcenter_Key},Tool_Var=${Tool_Var},Run_Quantity=${Run_Quantity},Run_Date=${Run_Date}`
    );
    const someRows = await conn.query(
      "call InsToolLifeHistoryV2(?,?,?,?,@Tool_Life_Key,@Return_Value); select @Tool_Life_Key as pTool_Life_Key,@Return_Value as pReturn_Value",
      [CNC_Approved_Workcenter_Key, Tool_Var, Run_Quantity, Run_Date]
    );
    let returnValue = someRows[1][0].pReturn_Value;
    let toolLifeKey = someRows[1][0].pTool_Life_Key;
...
async function UpdateCNCToolOpPartLife(
  CNC_Approved_Workcenter_Key,
  Set_No,
  Block_No,
  Current_Value,
  ...
  try {
    conn = await pool.getConnection();
    console.log(
      `In UpdateCNCToolOpPartLife with params CNC_Approved_Workcenter_Key=${CNC_Approved_Workcenter_Key},Set_No=${Set_No},Block_No=${Block_No},Current_Value=${Current_Value},Running_Total=${Running_Total},Last_Update=${Last_Update}`
    );
    ...
    const someRows = await conn.query(
      "call UpdateCNCToolOpPartLife(?,?,?,?,?,?,@ReturnValue); select @ReturnValue as pReturnValue",
      [
        CNC_Approved_Workcenter_Key,
        Set_No,
        Block_No,
...
async function UpdateCNCToolOpPartLifeV2(
  CNC_Approved_Workcenter_Key,
  Tool_Var,
  Current_Value,
  Running_Total,
  ...
  try {
    conn = await pool.getConnection();
    console.log(
      `In UpdateCNCToolOpPartLifeV2 with params CNC_Approved_Workcenter_Key=${CNC_Approved_Workcenter_Key},Tool_Var=${Tool_Var},Current_Value=${Current_Value},Running_Total=${Running_Total},Last_Update=${Last_Update}`
    );
    // const someRows = await conn.query('call UpdateCNCToolOpPartLifeCurrentValue(1,1,1,6,"2020-08-25 10:17:55",@ReturnValue); select @ReturnValue as pReturnValue');
    const someRows = await conn.query(
      "call UpdateCNCToolOpPartLifeV2(?,?,?,?,?,@ReturnValue); select @ReturnValue as pReturnValue",
      [
        CNC_Approved_Workcenter_Key,
        Tool_Var,
        Current_Value,
    ...
    let returnValue = someRows[1][0].pReturnValue;
    console.log(`UpdateCNCToolOpPartLifeV2.returnValue=${returnValue}`);
  } catch (err) {
    // handle the error
    console.log(`Error =>${err}`);
  } finally {
...
async function InsAssemblyMachiningHistory(
  ...
  try {
    ...
    let Assembly_Machining_History_Key =
      someRows[1][0].pAssembly_Machining_History_Key;
    console.log(`InsAssemblyMachiningHistory.returnValue=${returnValue}`);
    console.log(
      `InsAssemblyMachiningHistory.Assembly_Machining_History_Key=${Assembly_Machining_History_Key}`
    );
  } catch (err) {
    // handle the error
    console.log(`Error =>${err}`);
...
function main() {
  ...
  mqttClient.on("connect", function () {
    ...
    mqttClient.subscribe("InsToolLifeHistoryV2", function (err) {
      if (!err) {
        common.log("Tracker13319 has subscribed to: InsToolLifeHistoryV2");
      }
    }); //UpdateCNCToolOpPartLife
    ...
    mqttClient.subscribe("UpdateCNCToolOpPartLifeV2", function (err) {
      if (!err) {
        common.log("Tracker13319 has subscribed to: UpdateCNCToolOpPartLifeV2");
      }
    });
    mqttClient.subscribe("InsAssemblyMachiningHistory", function (err) {
      if (!err) {
        common.log(
          "Tracker13319 has subscribed to: InsAssemblyMachiningHistory"
        );
      }
    });
  });
  ...
  mqttClient.on("message", function (topic, message) {
    ...
    switch (topic) {
      case "InsToolLifeHistory":
        InsToolLifeHistory(
          obj.CNC_Approved_Workcenter_Key,
          obj.Set_No,
          obj.Block_No,
          obj.Run_Quantity,
          obj.Run_Date
      ...
      case "UpdateCNCToolOpPartLife":
        ...
        break;
      case "UpdateCNCToolOpPartLifeV2":
        UpdateCNCToolOpPartLifeV2(
          obj.CNC_Approved_Workcenter_Key,
          obj.Tool_Var,
          obj.Current_Value,
          obj.Running_Total,
